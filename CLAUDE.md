...
Navigation from the idle home screen is minimalist: two bare corner icons — **History** (bottom-right) and a **menu** button (bottom-left) that opens the `HomeMenu` modal with **Insights** and **Preferences**. Every sub-page (`history`, `historyDetail`, `insights`, `preferences`) has its own in-view "Home" back button. Back navigation is tracked by a `navHistory` ref in `Frame`.
...
- **`preferences`** — `PreferencesView`, a lightweight settings page reached from the home `HomeMenu`. Currently only lets the person edit the name Margo uses for them (persisted via `useOnboarding.setName`).
...
- **History & insights:** `HistoryView`, `EntryDetailView`, `InsightsView`, `PreferencesView`, `HomeMenu` (the bottom-left home modal → Insights / Preferences), `entryFormat.ts` (date/time/duration helpers), `insightsModel.ts`, `graphModel.ts`.
...
- **Never use `git stash` / `git stash pop`.** Multiple agents often work in this repo in parallel, and stashing silently moves another agent's in-flight changes onto a shared stack — pop then restores the wrong files into the wrong working tree. Leave changes in place; if you need a clean state, work on the specific files directly instead of stashing.
- **Keep `CLAUDE.md` and `README.md` in sync with your changes — but only when necessary.** After making a change, update these docs if (and only if) the change makes something they describe inaccurate (e.g. new/removed commands, env vars, routes, screens, or architecture). Don't touch them for changes they don't describe (small refactors, styling tweaks, bug fixes that don't alter documented behavior). Keep it short and concise. Do not bloat the Claude files.

## Conventions

- **Styling is CSS Modules** — one co-located `*.module.css` per component, imported as `styles` and referenced via `styles.foo` (use the `cx(...)` helper in `src/cx.ts` to join conditional/compound classes). Shared design tokens (gradients, core colors, the Inter font stack) live as CSS custom properties in `:root` in `src/index.css` (`--grad-screen`, `--grad-cta`, `--ink`, `--font-sans`, …) — reuse them via `var(...)` rather than re-hardcoding hexes/gradients. `src/index.css` also holds the base reset (replacing what was Tailwind's preflight) and the global `.btnReset` / `.focusRing` utility classes. The exact values still come verbatim from the Figma design (precise hexes, pixel sizes, gradients); match that fidelity when editing and don't invent new tokens unless asked. **Dynamic values stay inline** as `style={{…}}` when they depend on runtime state/props (e.g. computed frame scale/size in `Frame`, per-node positions/opacity in `EntryGraph`); `motion/react` animation props (`variants`/`animate`/`whileTap`/`transition`) stay as JS objects in TSX and are never moved to CSS.
